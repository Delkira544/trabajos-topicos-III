...
def generate_category_rules(items: list[dict]) -> list[dict]:
    ...
    for cat, total in counts.items():
        ...
        rules.append({"category": cat, "min": minimo, "max": maximo})
    return rules
...
def generate_dependencies(
    ...
    return [{"id_item": item, "id_dependency": req} for item, req in sorted(deps)]
...
def print_summary(
    ...
    total_peso = sum(it["weight"] for it in items)
    total_volumen = sum(it["volume"] for it in items)
...
def generate_instance(
    ...
    write_csv(
        ...
        ["id", "value", "weight", "volume", "category"],
        items,
    )
    write_csv(
