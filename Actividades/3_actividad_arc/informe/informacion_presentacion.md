...
**Memoria consumida** (ej: large + pop 16384):
- Población: 16384 × 10000 = 163.8 MB
- Offspring: otros 163.8 MB
- Total ≈ 328 MB (cómodo dentro de 8 GB VRAM)
...
- ❌ **Antes**: población completa D→H cada generación (ej: 16384×10000 = 163 MB por gen × 300 = ~49 GB)
- ✅ **Ahora**: solo escalares (~10 bytes/ind) cada generación + cromosoma del mejor al final (~n_items bytes)

### Para decir en la exposición
> "La población se mantiene siempre en GPU. Solo transferimos los fitness y flags de validez cada generación. El cromosoma del mejor individuo se descarga una sola vez al final. Esto reduce la transferencia acumulada de ~49 GB a ~6 MB, haciendo viable la paralelización."
...
**Resultado**: CUDA Optimized es **más lento** que CUDA Basic en instancias grandes (hasta 67% más lento en large).
...
|-------------|-------------|-------------|
| **EXP 1** | 3 instancias × 3 poblaciones × 3 variantes × 10 semillas | 269 |
| **EXP 2** | Efecto del bloque (32,64,128,256) × 2 variantes × 10 semillas | 80 |
| **EXP 3** | Speed-up = T_seq / T_GPU (derivado de EXP 1) | 269 |
...
|----------|----------------|
| Sequential | 1,177,400 ms (~19.6 min) |
| CUDA Basic | 64,055 ms (~1 min) |
| CUDA Optimized | 78,410 ms (~1.3 min) |

### Speed-up (CUDA Basic vs Sequential):
| Instancia | pop=1024 | pop=4096 | pop=16384 |
|-----------|----------|----------|-----------|
| small | 3.18× | 9.90× | 19.76× |
| medium | 23.72× | 55.03× | 133.22× |
| large | 6.44× | 18.38× | --- |

### Desglose de tiempo GPU (large, pop 4096, CUDA Basic):
| Componente | Tiempo (ms) | % del total |
|------------|-------------|-------------|
| Kernel reproducción | 62,375 | 99.2% |
| Kernel fitness | 502 | 0.8% |
| Transferencia H→D | 6.7 | ~0.01% |
| Transferencia D→H | 13.5 | ~0.02% |

### Para decir en la exposición
> "El speed-up máximo fue de 133× en la instancia mediana con población 16384. El kernel de reproducción domina el 99.2% del tiempo GPU, y las transferencias son prácticamente despreciables (0.03% del tiempo total). La variante optimizada resultó más lenta que la básica en instancias grandes, lo cual es un hallazgo importante."

---

## Diapositiva 12 — Efecto del Bloque y Población

### EXP 2: Tamaño de bloque (large, pop 4096)
| Variante | bs=32 | bs=64 | bs=128 | bs=256 |
|----------|-------|-------|--------|--------|
| CUDA Basic | 64,395 | **57,641** | 64,344 | 57,686 |
| CUDA Optimized | 84,748 | 81,554 | 79,329 | **78,395** |

- Óptimo CUDA Basic: **bs=64**
- Óptimo CUDA Optimized: **bs=256**

### EXP 4: Efecto de población (large)
| Variante | pop=1024 | pop=4096 | pop=16384 |
|----------|----------|----------|-----------|
| Sequential | 296,339 ms | 1,177,400 ms | --- |
| CUDA Basic | 45,996 ms | 64,055 ms | 128,464 ms |
| CUDA Optimized | 76,842 ms | 78,410 ms | 123,895 ms |

**Fitness mejora con población**: 0.1948 → 0.1965 → 0.1970

### Para decir en la exposición
> "El bloque óptimo fue 64 para CUDA Basic y 256 para Optimized. Las diferencias son marginales, alrededor de ±10%. El fitness mejora levemente al aumentar la población, pero a costa de más tiempo de cómputo."
...
| Transferencia masiva D→H cada gen | ~49 GB acumulados | Lazy download del mejor |
| `reduce_best_block/warp` nunca llamados | Código muerto | Eliminados del flujo |

### Para decir en la exposición
> "Encontramos y corregimos varios bugs durante el desarrollo. El más crítico era la transferencia masiva de población completa cada generación, que agregaba ~49 GB de tráfico PCIe y destruía cualquier speed-up potencial."
...
### Mostrar en CSV (tener abierto `results/christian/exp1_main_design.csv`):
- [ ] Una fila completa con las 15 columnas
...
1. **Speed-up masivo**: hasta 133× en instancia mediana con GPU vs CPU.
2. **Cuello de botella**: kernel de reproducción (99.2% del tiempo GPU) por divergencia de warps en la reparación.
3. **Optimizaciones contraproducentes**: shared memory y streams empeoraron el rendimiento en instancias grandes.
4. **Transferencias despreciables**: solo 13.5 ms acumulados D→H en 300 generaciones.
5. **Bloque óptimo**: 64 para CUDA Basic, 256 para CUDA Optimized.
...
- **¿Cuánta VRAM usa?** Para large+pop16384: ~328 MB (población + offspring). Sobran ~7.7 GB en una RTX 4060.
...
- **¿Cuánto tiempo toman las transfers?** ~13.5 ms acumulados en 300 generaciones (<0.03% del total).
