...
int main(int argc, char** argv)
{
      ...
      config.variant == "islands_parallel")
  {
    ...
    std::cout << "Migration interval: "  << config.migration_interval << std::endl;
  }

  if (config.variant == "cuda_basic" || config.variant == "cuda_optimized")
  {
    std::cout << "Block size (CUDA): " << config.block_size << std::endl;
  }
  ...
  try
  {
    ...
    solver_config.block_size         = config.block_size;   // <-- NUEVO

    // Crear y ejecutar solver
    auto solver = SolverFactory::create(config.variant, instance, solver_config);
    solver->run();
    ...
    {
      if (best.chromosome[i]) { std::cout << i << " "; count++; }
    }
